	...
	function hoverAnimStart(inc){
		if(inc){
			if(!busy && !rhover &&!lhover && !p3drag && opts.curr+2 <= opts.pTotal-2){
				...
				p3.addClass('b-grab');
				if(opts.closed && opts.autoCenter && opts.curr == 0){
					p3.stop().animate({'left':opts.pWidth-70, 'width':60}, 500, opts.easing);
				}else{
					p3.stop().animate({'left':opts.width-70, 'width':60}, 500, opts.easing);
				}
			...
			if(!busy && !lhover && !rhover && !p0drag && opts.curr-2 >= 0){
				...
				p0.stop().animate({left:10, width:60}, 400, opts.easing);
